...
"""
安装amazfit就可以用手机注册。然后用zeep life(原小米运动) 绑定支付宝和微信就好
"""
import requests
import time
import re
...
class Sport:
    ...
    def login(self):
        ...
        :return:
        """
        url1 = "https://api-user.huami.com/registrations/+86" + self.phone + "/tokens"
        ...
        r1 = requests.post(url1, data=data1, headers=self.headers, allow_redirects=False)
        ...
        r2 = requests.post(url2, data=data2, headers=self.headers).json()
    ...
    def update_step(self, login_token, userid):
        """
        修改步数
        :param login_token: 登录获得的token
        :param userid: 登录获取的用户id
        :return:
        """
        t, app_token, today = self.get_time(), self.get_app_token(login_token), time.strftime("%F")
        ...
        data_json = re.sub(findstep.findall(data_json)[0], str(self.step), str(data_json))
    ...
    def mian(self):
        ...
        # 修改步数
        self.update_step(login_token, userid)


if __name__ == '__main__':
    Sport('13565826354', 'ycy12569800', 1065).mian()
